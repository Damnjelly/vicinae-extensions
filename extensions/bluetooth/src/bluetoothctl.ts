...
export class Bluetoothctl {
	...
	scan(state: "on" | "off") { this.send(`scan ${state}`); }
	...
	async handleDeletedDevice(line: BluetoothctlLine): Promise<Device | null> {
		if (line.type !== LineType.DeviceDeleted) throw new Error("Invalid line type");
		return await Bluetoothctl.getDeviceInfo(line.device.mac);
	}
...
export async function pairToDevice(device: Device): Promise<void> {
	...
	return new Promise<void>((resolve, reject) => {
		...
		const handlePairingLine = async (line: BluetoothctlLine) => {
			if (line.type === LineType.PairingSuccess) {
				...
				await bt.handlePasskeyConfirmation(
					line,
					device,
					PairingContext.OutgoingPairing
				);
			}
		};
...
export async function connectToDevice(device: Device): Promise<void> {
	...
	return new Promise<void>((resolve, reject) => {
		...
		const handleConnectionLine = async (line: BluetoothctlLine) => {
			if (line.type === LineType.ConnectionSuccess) {
				clearTimeout(timeout);
				bt.kill();
				await showToast({
					style: Toast.Style.Success,
				...
				resolve();
			} else if (line.type === LineType.ConnectionFailure) {
				clearTimeout(timeout);
				bt.kill();
				await showToast({
					style: Toast.Style.Failure,
...
export async function disconnectFromDevice(device: Device): Promise<void> {
	...
	return new Promise<void>((resolve, reject) => {
		...
		const handleDisconnectLine = async (line: BluetoothctlLine) => {
			if (line.type === LineType.DisconnectSuccess) {
				clearTimeout(timeout);
				bt.kill();
				await showToast({
					style: Toast.Style.Success,
				...
				resolve();
			} else if (line.type === LineType.DisconnectFailure) {
				clearTimeout(timeout);
				bt.kill();
				await showToast({
					style: Toast.Style.Failure,
...
export async function removeDevice(device: Device): Promise<void> {
	...
	return new Promise<void>((resolve, reject) => {
		...
		const handleRemoveLine = async (line: BluetoothctlLine) => {
			if (line.type === LineType.RemoveSuccess) {
				clearTimeout(timeout);
				bt.kill();
				await showToast({
					style: Toast.Style.Success,
				...
				resolve();
			} else if (line.type === LineType.RemoveFailure) {
				clearTimeout(timeout);
				bt.kill();
				await showToast({
					style: Toast.Style.Failure,
...
export async function trustDevice(device: Device): Promise<void> {
	...
	return new Promise<void>((resolve, reject) => {
		...
		const handleTrustLine = async (line: BluetoothctlLine) => {
			if (line.type === LineType.TrustSuccess) {
				clearTimeout(timeout);
				bt.kill();
				await showToast({
					style: Toast.Style.Success,
				...
				resolve();
			} else if (line.type === LineType.TrustFailure) {
				clearTimeout(timeout);
				bt.kill();
				await showToast({
					style: Toast.Style.Failure,
